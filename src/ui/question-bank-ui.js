...
export function createBankPanel({
  ...
  // 依頼文の締め（全形式で共通）。「1行1問」だけだと全問を1行にまとめて返してくることが
  // あるため、改行を明示する。コードブロックで囲ませるのは、チャット画面でタブと改行が
  // そのまま保たれ、コピーしやすいため（記号行は取り込み側で読み飛ばす）
  const PROMPT_TAIL =
    '問題ごとに必ず改行してください。出力全体はコードブロック（```）で囲み、' +
    '前置き・番号・見出し・空行は入れないでください。'
  ...
  const PASTE_FORMATS = {
    buzzer: {
      ...
      sample: '日本の首都は？\t東京\n富士山の高さは？\t3776m\tm単位で',
      prompt:
        'クイズを20問作ってください。1問につき1行、各行は「問題」「答え」「補足メモ」を' +
        'タブ文字で区切って書き、' + PROMPT_TAIL,
    },
    ox: {
      columns: '問題 → ○ か × → メモ',
      sample: 'カンガルーは後ろ向きに歩けない\t○\n万里の長城は宇宙から見える\t×\t有名な俗説',
      prompt:
        '○×クイズを20問作ってください。1問につき1行、各行は「問題」「正解（○か×）」「補足メモ」を' +
        'タブ文字で区切って書き、' + PROMPT_TAIL,
    },
    choice4: {
      ...
      prompt:
        '4択クイズを20問作ってください。1問につき1行、各行は' +
        '「問題」「選択肢1」「選択肢2」「選択肢3」「選択肢4」「正解番号（1〜4）」「補足メモ」を' +
        'タブ文字で区切って書き、' + PROMPT_TAIL,
    },
  }
